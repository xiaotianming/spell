...
class LogParser:
    ...
    def SimpleLoopMatch(self, logClustL, seq):
        ...
        for logClust in logClustL:
            num = num + 1
            if float(self.seqlens1(logClust.logTemplate)) < 0.5 * self.seqlens2(seq):
                continue
    ...
    def parse(self, logname):
        ...
        print('nenum:', self.nenum)
        print('netime:{!s}'.format(self.netime))
        self.preciseseq(logCluL,'hpc.log')
...
t=LogParser('./logs-collection','./result/','<Head> 1 <Content>',0.5,[],False)
t.parse('HPC.log')